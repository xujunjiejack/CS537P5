...
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){
    ...
    wsect(x, buf);
      //int* indir = (int*) (long) (xint (din.addrs[NDIRECT]));
      if (fbn < NDIRECT) {
          din.sum[fbn] = adler32((void *) buf, 512);
      } else {
          fbn -= NDIRECT;
          // TODO: Now we know how to write data to it.
          //printf("%p\n", indir);
          //printf("%x, size of uint %d\n", indir, sizeof(uint));
          indirect[fbn + NINDIRECT] = adler32((void *) buf, 512);

          printf("In iappend fbn is %d, ", fbn + NDIRECT);
          int i;
          for (i = 0; i< 7; i++){
              printf("%d, ", din.addrs[i]);
          }
