...
iupdate(struct inode *ip)
{
  ...
  dip->nlink = ip->nlink;
  dip->size = ip->size;
  dip->indirect_block = ip->indirect_block;
...
//    cprintf("end bmapping\n");
  ...
  if(bn < NINDIRECT){
    ...
    if((addr = ip->indirect_block) == 0)
      ip->indirect_block = addr = balloc(ip->dev);
...
itrunc(struct inode *ip)
{
  ...
  if(ip->indirect_block){
    bp = bread(ip->dev, ip->indirect_block);
    a = (uint*)bp->data;
    ...
    brelse(bp);
    bfree(ip->dev, ip->indirect_block);
