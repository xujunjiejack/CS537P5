...
iupdate(struct inode *ip)
{
  ...
  memmove(dip->sum, ip->sum, sizeof(ip->sum));
  bwrite(bp);
  brelse(bp);
}
...
ilock(struct inode *ip)
{
  ...
  if(!(ip->flags & I_VALID)){
    ...
    memmove(ip->sum, dip->sum, sizeof(ip->sum));
    brelse(bp);
    ip->flags |= I_VALID;
    if(ip->type == 0)
      panic("ilock: no type");
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    uint bn = off/BSIZE;
    indir_buf = bread(ip->dev, ip->addrs[NDIRECT]);
    uint* indir = (uint*)indir_buf->data;
      uint sum;
      sum = adler32(bp->data, BSIZE);
      //cprintf("checking sum %d\n", sum);
      ...
      if (bn < NDIRECT) {
        ...
        if (ip->sum[bn] != sum) {
            ...
            cprintf("Sum should be :%d, but it gets: %d\n",
                    ip->sum[bn], sum);
            return -1;
        };
     }
          ...
          bn -= NDIRECT;
            if (indir[bn + NINDIRECT] != sum) {
                brelse(bp);
                brelse(indir_buf);
                cprintf("Error: checksum mismatch, block %d\n",
                        bn + NDIRECT);
                cprintf("Sum should be :%d, but it gets: %d\n",
                        indir[bn + NINDIRECT], sum);
                return -1;
            }
     }
    brelse(bp);
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    indir_buf = bread(ip->dev, ip->addrs[NDIRECT]);
    indir = (uint*) indir_buf->data;

    if (bn < NDIRECT){
        ip->sum[bn] = adler32(bp->data, BSIZE);
        iupdate(ip);
    } else{
        bn -= NDIRECT;
        if (bn < NINDIRECT){
            indir[NINDIRECT + bn] = adler32(bp->data, BSIZE);
        }
